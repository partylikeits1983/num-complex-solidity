 ...
 *   - Uses 32 bits instead of 16 bits for improved accuracy
...
library Trigonometry {
    ...
    uint256 constant MAX_ACCURATE_ANGLE = 1e28;

    error AngleOutOfBounds(uint256 angle);
    ...
    function sin(uint256 angle) internal pure returns (int256) {
        ...
        return _lookup(table, _toCycle(angle));
    }

    /**
     * @notice Returns sine and cosine while loading the lookup table and normalizing the angle once.
     * @param angle Angle in radians scaled by 1e18.
     * @return sine Sine scaled by 1e18.
     * @return cosine Cosine scaled by 1e18.
     */
    function sinCos(uint256 angle) internal pure returns (int256 sine, int256 cosine) {
        bytes memory table = SIN_TABLE;
        uint256 cycleAngle = _toCycle(angle);
        sine = _lookup(table, cycleAngle);
        cosine = _lookup(table, (cycleAngle + ANGLES_IN_CYCLE / 4) % ANGLES_IN_CYCLE);
    }

    /**
     * @notice Return the cosine of a value, specified in radians scaled by 1e18
     * @dev Uses the identity cos(x) = sin(x + pi/2) in the algorithm's bounded cycle representation.
     * @param angle Angle to convert
     * @return Result scaled by 1e18
     */
    function cos(uint256 angle) internal pure returns (int256) {
        bytes memory table = SIN_TABLE;
        uint256 cycleAngle = _toCycle(angle);
        return _lookup(table, (cycleAngle + ANGLES_IN_CYCLE / 4) % ANGLES_IN_CYCLE);
    }

    function _toCycle(uint256 angle) private pure returns (uint256) {
        if (angle > MAX_ACCURATE_ANGLE) revert AngleOutOfBounds(angle);
        return (ANGLES_IN_CYCLE * (angle % TWO_PI)) / TWO_PI;
    }
    ...
    function _lookup(bytes memory table, uint256 angle) private pure returns (int256) {
        unchecked {
            ...
            if (!isOddQuadrant) index = SINE_TABLE_SIZE - 1 - index;

            uint256 packedEntries;
            uint256 offset = (index + 2) * ENTRY_BYTES;
            assembly ("memory-safe") {
                packedEntries := mload(add(table, offset))
            }
